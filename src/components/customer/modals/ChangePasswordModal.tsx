...
import { showSuccessAlert } from "../../../utils/swalConfig";
...
export default function ChangePasswordModal({ isOpen, onClose }: ChangePasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Handle logic
    await showSuccessAlert("Kata Sandi Berhasil Diperbarui!", "Kata sandi berhasil diperbarui");
    onClose();
  };
        ...
        <div className="p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-4 flex gap-3">
              <button type="button" onClick={onClose} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-4 rounded-2xl border border-white/10 transition-all">
                Batal
              </button>
              <button type="submit" className="flex-1 bg-orange-600 hover:bg-orange-500 text-white font-black uppercase tracking-widest py-4 rounded-2xl shadow-lg shadow-orange-900/20 active:scale-[0.98] transition-all">
                Simpan
              </button>
            </div>
          </form>
