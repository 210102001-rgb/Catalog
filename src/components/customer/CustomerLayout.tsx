...
export default function CustomerLayout({ children, activePage, title = "", showSearch = false, onSearch }: CustomerLayoutProps) {
  ...
  return (
    <div className="flex h-screen w-full bg-background-dark overflow-hidden font-sans">
      ...
      <Sidebar title="ReklameKu" subtitle="Portal Pelanggan" sections={navSections} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        {!isSidebarOpen && (
          <button
            ...
            className="md:hidden absolute top-4 left-3 z-40 size-12 flex items-center justify-center text-white bg-white/10 rounded-xl border border-white/10 backdrop-blur-lg shadow-lg active:scale-95 transition-all"
        ...
        <CustomerHeader title={title} showSearch={showSearch} onSearch={onSearch} onCartClick={() => setIsCartOpen(true)} onNotifClick={() => setIsNotifOpen(true)} onLogoutClick={() => setIsLogoutOpen(true)} />

        <div className="flex-1 min-w-0 overflow-y-auto relative">{children}</div>
      </div>
