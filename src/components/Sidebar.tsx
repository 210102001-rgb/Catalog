    ...
    return (
        <>
            ...
            <aside className={`
                ...
                w-64 bg-[#111a22] border-r border-[#233648] flex flex-col flex-shrink-0 h-full
                ...
                <div className="p-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <h1 className="text-white text-base font-bold leading-normal tracking-tight">
                                {title}
                            </h1>
                ...
                <nav className="flex-1 px-4 py-2 flex flex-col gap-2 overflow-y-auto">
                    {sections.map((section, sectionIdx) => (
                        <div key={sectionIdx} className={section.title ? 'mt-4 pt-4 border-t border-[#233648]' : ''}>
                            {section.title && (
                                <p className="px-3 text-xs font-semibold text-[#92adc9] uppercase tracking-wider mb-2">
                                    {section.title}
                                </p>
                            )}
                            {section.items.map((item, itemIdx) => {
                                const content = (
                                    <>
                                        ...
                                        {item.badge && (
                                            <span
                                                className={`ml-auto ${item.badgeColor || 'bg-primary'
                                ...
                                const className = `flex items-center gap-3 px-3 py-3 rounded-lg transition-all w-full text-left ${item.active
